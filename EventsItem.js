//add sr
Vue.component('EventsItem',{
  template:`<div name="EventsItem" :class="[event.active&&'bg-attention-warning']">
    ...
    <template v-if="open">
      ...
      <info-text-sec v-if="detailsText" title="Дополнительно" :text="detailsText" class="margin-bottom-4px"/>
      
      <SiebelServiceRequest v-if="event.details?.siebel_id" :srNumber="event.details.siebel_id" class="margin-left-right-16px"/>
      ...
      <template v-if="event.ops&&(event.ops.name||event.ops.login)">
        ...
        <PhoneCall v-if="event.ops.phone" :phone="event.ops.phone" :descr="event.ops.name||event.ops.login" showSendSms class="margin-bottom-4px margin-left-right-16px"/>
