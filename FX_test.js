...
if(document.title!='Inetcore+'&&(window.location.href.includes('https://fx.mts.ru')||window.location.href.includes('http://inetcore.mts.ru/fix')||window.location.href.includes('http://pre.inetcore.mts.ru/fix'))){
	...
	*/
	...
	Vue.component('session-history-modal',{
				...
				<div>
					<div>
						<div class='d-center-x px-16 py-16'>
							<text-field-el :value='history.start' label='Начало' type='date'  v-model='history.start' class='mr-8'/>
							<text-field-el :value='history.end' label='Конец' type='date' v-model='history.end' class='ml-8'/>
						</div>
					...
					<template v-for="entry in history.data">
						...
						<div class='py-16'>
							...
							<info-value label='IP' :value='entry.ip' type='large' :whithLine=true></info-value>
							<info-value label='MAC' :value='entry.mac' type='large' :whithLine=true></info-value>
							<info-value label='NAS' :value='entry.nas' type='large' :whithLine=true></info-value>
							<info-value label='Тип трафика' :value='entry.catdescr||entry.log' type='large' :whithLine=true></info-value>
						</div>
					</template>
				</div>
		...
		computed:{
			message(){
				if(this.history.start.toLocaleDateString()==this.history.end.toLocaleDateString()){
					return {
						text:`Завершенных сессий ${history.start.toLocaleDateString()} нет`,
						type:'info'
					}
				};
				if(this.history.start > this.history.end){
					return {
						text:'Выбраны неверные даты',
						type:'warn'
					}
				};
				return {
					text:'Завершенных сессий в указзаный период нет',
					type:'info'
				}
			},
		},
		methods:{
			...
			load:function(){
				...
				const params={
					login:this.session.params.login,
					serverid:this.session.params.serverid,
					vgid:this.session.params.vgid,
					descr:this.session.params.descr,
					dtfrom:this.history.start,
					dtto:this.history.end
				};
				httpPost('/call/aaa/get_sessions',params).then(data=>{
					this.history.data = data.rows;
					this.loading = false;
				}).catch(e=>{
					console.error(e);
					this.loading=false;
