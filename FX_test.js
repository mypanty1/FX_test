    ...
    <div>
      ...
      <div>
        <div class='d-center-x px-16 py-16'>
          <text-field-el :value='logs.date' label='дата' type='date'  v-model='logs.date'/>
        </div>
      ...
      <template v-if='logs.data' v-for="entry in logs.data">
        ...
        <div class='py-16'>
          <div class='font--13-500-140 tone-900 px-16'> {{ entry.logs }} ({{ entry.count }})</div>
		  ...
		  <span style="margin-left:1rem;">{{entry.log}}</span>
        </div>
      </template>
    </div>
					...
					<div v-if="showNav">
						...
						<card-block>
							...
							<div style="text-align:right;padding-right:1em;margin-top:-2em;">
								<input type="button" id="btn_downloadPL" @click="generate(site.id)" style="font-family:arial;font-size:8pt;padding:1px;opacity:0;" value="план-схема">
							</div>
							<devider-line/>
