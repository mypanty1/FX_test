...
(function(id=`EventsMap-css`){
  ...
  el.appendChild(document.createTextNode(`
    ...
    .transition_sidebar_right_slide-enter-active{
      transition:400ms;
    }
    .transition_sidebar_right_slide-leave-active{
      transition:200ms;
    }
    .transition_sidebar_right_slide-enter{
      transform:translate(50%,0);
      opacity:0;
    }
    .transition_sidebar_right_slide-enter-to{
      opacity:1;
    }
    .transition_sidebar_right_slide-leave-to{
      transform:translate(50%,0);
      opacity:0;
    }
  `));
...
const EVENTS_MAP=new function(){
  this.CACHE_1HOUR=60;
  this.CACHE_1DAY=this.CACHE_1HOUR*24;
  this.CIRCLE_RADIUS_M=32;
  
  this.GeocodeResult=class {
  ...
  this.FeatureCollection=class {
    constructor(features){
      this.type='FeatureCollection'
      this.features=features
    }
  }
  this.PointGeometry=class {
    constructor(coordinates){
      this.type='Point'
      this.coordinates=coordinates
    }
  }
  this.PolygonGeometry=class {
    constructor(coordinates){
      this.type='Polygon'
      this.coordinates=coordinates
    }
  }
  this.CircleIconShape=class {
    constructor(radius){
      this.type='Circle'
  ...
  this.RectangleIconShape=class {
    constructor(half){
      this.type='Rectangle'
      this.coordinates=[[-half,-half],[half,half]]
    }
  }
  this.IconImageSizeOffset=class {
    #size;
  ...
  this.PortObjectInfo=class extends ObjectInfo {
    constructor(port={}){
      super('port',port.name)
      this.deviceName=port.device_name||''
    }
  }
  this.AbonObjectInfo=class extends ObjectInfo {
  ...
  this.nearCoordinates=function([lat=0,lon=0]=[],multipler=1){
    return [
  ...
  this.sortPointsToArea=function(points=[]){
  ...
  this.getIntId=function(){return parseInt((Math.random()*10000).toFixed().padStart(4,0)+Date.now())}
  this.mountHintView=function(properties={}){
    return new Vue({
      store,
  ...
  getters:{
    ...
    getDeviceAbons:(state,getters)=>(deviceName)=>{
      return getters.getDevicePorts(deviceName).reduce((abons,port)=>{
        for(const sub of port.subscriber_list||[]){
          abons.push({...new EVENTS_MAP.DevicePortAbonInfo(sub),...new EVENTS_MAP.DevicePortInfo(port)});
        };
        return abons
      },[])
    },
    ...
    getDeviceAbsentObjectsInfo:(state,getters)=>(deviceName)=>{
      return getters.getDevicePorts(deviceName).reduce((absentObjectsInfo,port)=>{
        if(!getters.getObjectInfo(port.name)){
          absentObjectsInfo[port.name]=new EVENTS_MAP.PortObjectInfo(port);
        };
        for(const sub of port.subscriber_list||[]){
          if(!getters.getObjectInfo(sub.account)){
            absentObjectsInfo[sub.account]=new EVENTS_MAP.AbonObjectInfo(port,sub);
          };
        };
    ...
    getDevicePortAbons:(state,getters)=>(deviceName,devicePortName)=>{
      ...
      return (port.subscriber_list||[]).map(sub=>{
        return {...new EVENTS_MAP.DevicePortAbonInfo(sub),...new EVENTS_MAP.DevicePortInfo(port)};
      });
    },
    ...
    getDevicePortAreaCoordinates:(state,getters)=>(deviceName,devicePortName)=>{
      ...
      return points.length>2?[EVENTS_MAP.sortPointsToArea(points),[]]:null;
    },
    ...
    async getSiteNodeInfo({getters,commit},nodeName){
      ...
      if(cache){
        ...
        try{
          ...
          if(response?.data?.id){
            localStorageCache.setItem(`siteNodeInfo-${nodeName}`,response.data,EVENTS_MAP.CACHE_1DAY);
      ...
      if(cache?.response){
        ...
        try{
          ...
          if(Array.isArray(response)){
            ...
            localStorageCache.setItem(`device_port_list/${deviceName}`,cache,EVENTS_MAP.CACHE_1HOUR);
    ...
    async getDevicePortInfo({getters,commit},{deviceName,devicePortName,trunk=false}={}){
      ...
      if(cache){
        ...
        try{
          ...
          if(Array.isArray(response)){
            localStorageCache.setItem(`port_info:cp/${devicePortName}`,response,EVENTS_MAP.CACHE_1HOUR);
...
Vue.component('EventsMapSidebarHeader',{
  template:`<div class="display-flex gap-4px align-items-center justify-content-space-between" :class="[reverse&&'flex-direction-row-reverse']">
...
Vue.component('EventsMapSidebarLeft',{
  template:`<div class="block-ffffff-28 display-flex flex-direction-column gap-4px" style="min-width:300px;max-width:400px;height:90vh;padding:4px;opacity:0.9;">
    <EventsMapSidebarHeader @close="$emit('close')">
      <div slot="title" class="display-flex gap-4px align-items-center">
        <span v-if="title" class="font--13-500 white-space-pre">{{title}}</span>
      </div>
    </EventsMapSidebarHeader>
...
Vue.component('EventsMapSidebarRight',{
  template:`<div class="block-ffffff-28 display-flex flex-direction-column gap-4px" style="min-width:300px;max-width:400px;height:90vh;padding:4px;opacity:0.9;">
    <EventsMapSidebarHeader reverse @close="$emit('close')">
      <div slot="title" class="display-flex gap-4px align-items-center">
  ...
  methods:{
    ...
    async initYmaps(){
      ...
      this.ymap.hint.events.add('open',(event)=>{
        ...
        this.ymap.hint.customView=EVENTS_MAP.mountHintView(event.originalEvent.target.properties.getAll());
      });
    },
    async getDevicesListAndAddToListBox(){
      try{
        ...
        if(Array.isArray(response)){
          this.$cache.setItem(cacheKey,response,EVENTS_MAP.CACHE_1HOUR);
    ...
    async getSampleAddressCoordinates(sample){
      if(!window.ymaps){return new EVENTS_MAP.GeocodeResult(sample)};
      this.geocodeLoading=true;
      const response=await window.ymaps.geocode(sample,{json:true,results:1});
      this.geocodeLoading=!true;
      return new EVENTS_MAP.GeocodeResult(sample,response)
    },
      ...
      try{
        if(!mapObjects[mapObjectId]){
          this.addMapObject(mapObjects[mapObjectId]=new window.ymaps.Placemark([latitude,longitude],{
            ...
            zIndex:EVENTS_MAP.ZINDEX_OLT,
            iconLayout:'default#image',
            iconImageHref:EVENTS_MAP.ICONS.OLT,
            iconShape:new EVENTS_MAP.CircleIconShape(24),
            ...new EVENTS_MAP.IconImageSizeOffset(48),
          }));
        }else{
    ...
    setMapObjectDevicePortAbon(deviceName,devicePortName,accountId){
      ...
      if(!latitude||!longitude){//если нет позиции ONT берем позицию PON
        ...
        const [_lat,_lon]=EVENTS_MAP.nearCoordinatesRand([_latitude,_longitude],1);
        latitude=_lat;
        longitude=_lon;
      };
      if(!latitude||!longitude){//если не позиции PON берем позицию OLT
        ...
        const [_latitude,_longitude]=coordinates;
        if(!_latitude||!_longitude){return};
        const [_lat,_lon]=EVENTS_MAP.nearCoordinatesRand([_latitude,_longitude],2);
        latitude=_lat;
        longitude=_lon;
      };
      ...
      try{
        if(!mapObjects[mapObjectId]){
          this.addMapObject(mapObjects[mapObjectId]=new window.ymaps.Placemark([latitude,longitude],{
            ...
            openEmptyHint:true,
            hintContentLayout:window.ymaps.templateLayoutFactory.createClass(`<div name="YMapsHintName"></div>`),
            zIndex:EVENTS_MAP.ZINDEX_ONT,
            iconLayout:'default#image',
            iconImageHref:EVENTS_MAP.ICONS.ONT_NORMAL_7B68EE99,
            iconShape:new EVENTS_MAP.RectangleIconShape(16),
            ...new EVENTS_MAP.IconImageSizeOffset(32),
          }));
    ...
    setMapObjectDevicePortPoint(deviceName,devicePortName){
      ...
      if(!latitude||!longitude){
        ...
        const [_latitude,_longitude]=coordinates;
        if(!_latitude||!_longitude){return};
        const [_lat,_lon]=devicePortAbonsCount?EVENTS_MAP.nearCoordinatesRand([_latitude,_longitude],1):EVENTS_MAP.nearCoordinates([_latitude,_longitude],1);
        latitude=_lat;
        longitude=_lon;
      };
      ...
      try{
        if(!mapObjects[mapObjectPointId]){
          this.addMapObject(mapObjects[mapObjectPointId]=new window.ymaps.Placemark([latitude,longitude],{
            ...
            openEmptyHint:true,
            hintContentLayout:window.ymaps.templateLayoutFactory.createClass(`<div name="YMapsHintName"></div>`),
            zIndex:EVENTS_MAP.ZINDEX_PON,
            iconLayout:'default#image',
            iconImageHref:EVENTS_MAP.ICONS.PON_0060F0,
            iconShape:new EVENTS_MAP.RectangleIconShape(18),
            ...new EVENTS_MAP.IconImageSizeOffset(36),
          }));
        ...
        const radius=this.getObjectInfo(devicePortName)?.radius||EVENTS_MAP.CIRCLE_RADIUS_M;
