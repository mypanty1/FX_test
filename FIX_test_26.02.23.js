...
Vue.component("port-actions",{
  ...
  template:`<card-block>
    ...
    <template v-if="open">
      ...
      <port-logs :disabled="disableAction.log" :port="port" :device="device"/>
...
Vue.component("port-logs", {
  //template: "#port-log-logs-template",
  template:` <section>
    <port-logs-modal ref="modal" :params-load="paramsLoad" v-bind="{port,device}"/>
    <link-block icon="log" text="Логи порта*" :disabled="disabled" @block-click="showLog" actionIcon="expand"/>
  </section>`,
  props: {
    disabled: {
      type: Boolean,
  ...
  computed: {
    paramsLoad() {
      const port = this.portParams;
      const device = this.deviceParams;
      return {
        port, device
      }
    },
...
Vue.component("port-logs-modal", {
  //template: "#port-logs-modal-template",
  template:`<modal-container-custom ref="modal" @open="openedEvent" :header="false" :footer="false" :wrapperStyle="{'min-height':'auto'}">
    <!--<modal-title title="Логи коммутатора" :subtitle="subtitle"/>-->
    <div class="display-flex align-items-center justify-content-space-between--- gap-4px margin-left-right-16px margin-bottom-16px">
      <switch-el class="width-40px" v-model="filter"/>
      <div class="font--13-500" v-if="filter">Логи по порту {{port.snmp_name}}</div>
      <div class="font--13-500" v-else>Логи по коммутатору {{device.ip}}</div>
    </div>
    
    <loader-bootstrap v-if="loading" text="получение логов с коммутатора"/>
    
    <div v-if="log.status=='success'" class="port-log-modal__list--- margin-left-right-8px display-flex flex-direction-column gap-4px">
      <div class="port-log-modal__elem--- font--12-400 margin-bottom-unset---" v-for="logLine in logs">{{logLine}}</div>
    </div>
  ...
  props: {
    paramsLoad:{type:Object,required:true},
    port:{type:Object,required:true},
    device:{type:Object,required:true},
  },
  data: () => ({
  ...
  computed: {
    ...
    subtitle(){
      ...
      if(this.filter){
        row.push(', порт',this.port.snmp_name)
      };
      return row.join(' ');
    },
    logs() {
      ...
      const ifName=` ${this.port.snmp_name} `;//Fiberhome, Huawei(+in=ev)
  ...
  methods: {
    ...
    async loadLogs() {
      ...
      this.log.status = "";
      try {
        const params = this.paramsLoad;
        const response = await httpPost("/call/hdm/log_short", params);
        if (response.message === "OK") {
