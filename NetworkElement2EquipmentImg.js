...
Vue.component('NetworkElement2',{
  template:`<section name="NetworkElement2">
    ...
    <card-block>
      ...
      <EquipmentImg :equipment_id="networkElement.system_object_id" v-if="username=='mypanty1'" showTopDevider autoOpenIfSrc/>
...
Vue.component('EquipmentImg',{
  template:`<div name="EquipmentImg" class="display-content">
    <devider-line v-if="showTopDevider"/>
    <link-block :actionIcon="open?'up':'down'" icon="сube" text="Внешний вид" type="large" @block-click="open=!open" class="my-m8"/>
  ...
  props:{
    ...
    showTopDevider:{type:Boolean,default:false},
    autoOpenIfSrc:{type:Boolean,default:false},
  },
  data:()=>({
    open:false,
    loading:false,
  ...
  computed:{},
  methods:{
    async getEquipmentImgSrc(){
      const {equipment_id}=this;
      if(!equipment_id){return};
      this.src='';
      ...
      this.loading=true;
      try{
        const response=await fetch(`https://script.google.com/macros/s/AKfycby26qbmgJjIAi1CGSj9sg9EBkn7mTsw5htXQk4G_DqUnncmuaBQn_wC54fYRHPaqMRa/exec?equipment_id=${equipment_id}`).then(resp=>resp.json());
        this.src=response?.src||'';
        this.$cache.setItem(`img_src/${equipment_id}`,response);
      }catch(error){
        console.warn('getEquipmentImgSrc.error',error)
      };
      this.loading=false;
    },
  },
