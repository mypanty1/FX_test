  ...
  beforeCreate(){
    if(this.$options.name == 'TechMaintenanceObjectPhotoGallery'){
      ...
      <div class="position-relative height-64px width-64px display-flex align-items-center justify-content-center">
        ...
        <button for="inputID" @click="$refs[inputID].click()" class="reset--button height-48px width-48px display-flex align-items-center justify-content-center cursor-pointer border-1px-solid-918f8f border-radius-4px">
      ...
      <button for="inputID" @click="$refs[inputID].click()" v-if="!countFiles" class="reset--button margin-auto height-64px display-flex align-items-center gap-8px">
        <span class="ic-24 ic-ip-mac-port tone-500"></span>
        <span class="font--13-500 tone-500">Прикрепите фото</span>
      </button>
      <div class="font--11-600 tone-500" v-if="$store.getters.userLogin == 'mypanty1'">test 2</div>
