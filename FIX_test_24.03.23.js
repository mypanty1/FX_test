    ...
    <template v-if="!loading&&rows.length">
      <template v-for="(row,i) of rows">
        <template v-if="row.mac">
          <info-value :key="i" :label="row.mac" :value="row.text" withLine type="medium"  data-ic-test="mac_port_security"/>
          <info-text-sec v-if="ouis[row.mac]" :text="ouis[row.mac]" style="margin-top:-8px;"/>
        </template>
        <info-value v-else :key="i" :label="row.text" value=" " type="medium" data-ic-test="mac_port_security"/>
      </template>
    </template>
