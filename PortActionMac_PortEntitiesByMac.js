...
Vue.component("PortActionMac",{
  template:`<section name="PortActionMac">
    ...
    <template v-if="!loading&&rows.length">
      <PortEntitiesByMac v-for="({text,mac},key) of rows" :key="key" v-bind="{text,mac,port,networkElement}" :oui="ouis[mac]" :mr_id="networkElement.region.mr_id" :region_id="networkElement.region.id" :noSession="rows.length>2"/>
    </template>
  ...
  methods:{
    async parse(rows){
      const {items,macs}=rows.reduce((result,_row)=>{//ffff.ffff.ffff,ff:ff:ff:ff:ff:ff,xxxx.ffff.ffff,
        ...
        const text=(mac?row.replace(mac,''):row).split(' ').filter(Boolean).join(' • ')
    ...
    async loadMacs(){
      ...
      this.text="";
      try{
        const response=await httpGet(buildUrl('port_mac_show',new DNM.DevicePortParams(this.networkElement,this.port),"/call/hdm/"));
    ...
    async clearMac(){
      ...
      try {
        ...
        const response=await httpGet(buildUrl('clear_macs_on_port',new DNM.DevicePortParams(this.networkElement,this.port),"/call/hdm/"));
...
Vue.component("PortEntitiesByMac",{
  ...
  computed:{
    ...
    serverid(){
      return {
        ...
        '29':[10],'30':[27],'31':[1001,112],'33':[70],'34':[110],'35':[9],'36':[47],'37':[3],'38':[55],'40':[6],'41':[131],'42':[137,240],'43':[116],'45':[66],
    ...
    isNsk(){return this.region_id==54},
    agentRemoteId_IP(){
      const arr=(`${this.sessionByMac?.remote_id||this.sessionByMac?.device||''}`.match(/.{2}/gi)||[]).map(b=>{
        b=b.padStart(2,0);
      ...
      return arr.length!=4?'':arr.map(s=>parseInt(s)).join('.');
    },
    diffIP(){
      const {isNsk,networkElement,agentRemoteId_IP}=this;
      return isNsk&&agentRemoteId_IP&&(networkElement.ip!=agentRemoteId_IP)
    },
    agentCircuitId_portNumber(){return parseInt(`${this.sessionByMac?.port||''}`)||0},
    diffPort(){
      const {isNsk,port,agentCircuitId_portNumber}=this;
      return isNsk&&agentCircuitId_portNumber&&(port.number!=agentCircuitId_portNumber)
    },
  ...
  methods:{
    ...
    async getSessionByMac(){
      ...
      this.$set(this.loads,key,true);
      try{
        const response=await AAAService.getXRadSessionByMac(serverid,mac);
        this.sessionByMac=response?.data?.[0]||null;
      }catch(error){
        console.warn('getSessionByMac.error', error);
      };
      this.$set(this.loads,key,false);
    },
    async stopGuestSession(){
      ...
      this.$set(this.loads,key,true);
      try{
        const response=await AAAService.stopXRadSessionByID(serverid,dbsessid);
        if(response?.data?.code==200){
          this.sessionByMac=null;
        };
      }catch(error){
        console.warn('stopGuestSession.error', error);
      };
      this.$set(this.loads,key,false);
    }
  },
