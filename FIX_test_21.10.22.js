...
Vue.component("site-devices",{//hide flats edit
  //template:"#site-devices-template",
    ...
    <template v-if="opened">
      ...
      <checkbox-el v-if='!!countNotMountDevices' :label="`Не установленное ${countNotMountDevices?'('+countNotMountDevices+')':''}`" v-model="showNotMountDevices" reverse class="px-8"/>
      <template v-if="loading.racks || loading.plints">
        <devider-line />
        <loader-bootstrap text="загрузка данных ДРС"/>
      </template>

      <site-devices-entrance v-else v-for="entranceData of schema" :key="entranceData.entrance.id"
        :ref="'site_devices_entrance_'+entranceData.entrance.id"
        :entranceData="entranceData"
        :showDrs="showDrs"
        :showNotMountDevices="showNotMountDevices"
        :entrances="entrances"
      ...
      <template v-if="devicesOutOfSiteOnlyByNiossAndGpon.length">
        ...
        <template v-if="opened_devicesOutOfSiteOnlyByNiossAndGpon">
          <template v-for="(device,i) of devicesOutOfSiteOnlyByNiossAndGpon">
            <devider-line v-if="i" class="mx-16"/>
            <device-info :device="device" :ports="getServePonPorts(device.name)" :ref="'device_info_'+device.nioss_id" :entrances="getServeEntrances(device.name)" showLocation class="py-8 b-w-1 b-s-ds b-c-grey mx-16"/>
          </template>
        </template>
      </template>
      
      <template v-if="countMountOutDevices||(countUnmountOutDevices&&showNotMountDevices)">
        <devider-line/>
        <title-main icon="warning" text="Место установки неизвестно" :text2="(countMountOutDevices+(showNotMountDevices?countUnmountOutDevices:0))||''" @open="openedOutDevices=!openedOutDevices"/>
        <template v-if="openedOutDevices">
          <template v-for="(device,i) of outDevices">
            <template v-if="!device.ne_status||(device.ne_status&&showNotMountDevices)">
              <devider-line v-if="i" class="mx-16"/>
              <device-info :device="device" :entrances="entrances" :ref="'device_info_'+device.nioss_id" showSessions showDisplayName class="py-8 b-w-1 b-s-ds b-c-grey mx-16"/>
            </template>
          </template>
        </template>
      </template>
      ...
      <template v-if="devicesOutOfSiteButMaybeWrong.length">
        ...
        <template v-if="opened_devicesOutOfSiteButMaybeWrong">
      ...
      <div v-if="openedActions" class="p-all d-flex-y g-4">
  ...
  computed:{
    devicesForFindPort(){//public, for find-port in site-du-content
    ...
    schema(){
      if(!(this.entrances&&this.devices&&this.plints&&this.racks)){return null};
      let schema={};
      for(let entrance of this.entrances){
        let entranceRacks=this.racks.filter(rack=>rack.entrance.number==entrance.number);
        let entrancePlints=this.plints.filter(plint=>plint.entrance_id===entrance.id);
        let outPlints=entrancePlints.filter(plint=>!entranceRacks.some(this.checkInsideRack(plint.name)));
        let racksWithData=entranceRacks.map(rack=>({
          rack,
          onlyDrs:/^CU/i.test(rack.name),
          devices:this.devices.filter(device=>rack.ne_in_rack.includes(device.name.replace('А','A'))),
          plints:this.plints.filter(plint=>plint.rack_id===rack.id),
        })).sort(({onlyDrs})=>onlyDrs?1:-1);
        let noDrsRacks=racksWithData.filter(({ onlyDrs }) => !onlyDrs);
        let noDrsDevices=noDrsRacks.some((data) => Boolean(data.devices.length || data.plints.length));
        let drsRacks=racksWithData.filter(({ onlyDrs }) => onlyDrs);
        let drsDevices=drsRacks.some((data) => Boolean(data.devices.length || data.plints.length));

        schema[entrance.number]={
          site: this.site,
          entrance,
          racks: racksWithData,
          outPlints,
    ...
    outDevices(){
      if(!this.racks.length){return this.devices};
      return this.devices.filter(device=>{
        return !this.racks.find(rack=>rack.ne_in_rack.includes(device.name.replace('А','A')))
      })
    },
    countMountOutDevices(){
      return this.outDevices.filter(device=>!device.ne_status).length;
    },
    countUnmountOutDevices(){
      return this.outDevices.filter(device=>device.ne_status).length;
    },
    toFlats(){
      return {
    ...
    countNotMountDevices(){
      return this.devices.filter(device=>typeof device.ne_status!=='undefined').length;
    },
    device_info_refs(){
